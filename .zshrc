...
# NOTE: need to fine tune this
# GIT terminal prompt config
git_branch() {
...
configure_prompt() {
    ...
    case "$PROMPT_ALTERNATIVE" in
        twoline)
            PROMPT='%F{%(#.blue.green)}┌──${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}(%B%F{%(#.red.blue)}%n'$prompt_symbol$'%m%b%F{%(#.blue.green)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}] $(if [ -d .git ]; then echo -e "GIT: ($git_color$(git_branch)%F{%(#.blue.green)})"; fi) %F{reset}%b%K{reset}%F{white}\n%F{%(#.blue.green)}└─%b%(#.%F{red}#.%F{blue}$)%b%F{reset} '
